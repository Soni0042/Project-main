...
function FourBhkPage() {
  const [properties, setProperties] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    fetch("http://localhost:3000/api/properties?bhk=4")
      .then((res) => res.json())
      .then((data) => {
  ...
  if (loading) return <p>Loading 4 BHK properties...</p>;
  if (error) return <p>Error: {error}</p>;

  return (
    <div style={styles.container}>
      <h1 style={styles.heading}>4 BHK Properties</h1>
